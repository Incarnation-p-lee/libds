...
{
    if (doubly_linked_list_structure_legal_p(list)
        && doubly_linked_list_structure_legal_p(previous)) {
...
{
    dp_assert(doubly_linked_list_structure_legal_p(list));
...
doubly_linked_list_create_i(void *val)
{
...
{
    dp_assert(doubly_linked_list_structure_legal_p(list));
    dp_assert(doubly_linked_list_structure_legal_p(node));
    dp_assert(!doubly_linked_list_contains_i_p(list, node));
...
{
    if (!doubly_linked_list_structure_legal_p(list)
        || !doubly_linked_list_structure_legal_p(node)) {
        return;
...
doubly_linked_list_node_copy(struct doubly_linked_list *node)
{
    ...
    if (!doubly_linked_list_structure_legal_p(node)) {
        return PTR_INVALID;
    } else {
...
doubly_linked_list_destroy(struct doubly_linked_list **list)
{
    ...
    if (!complain_null_pointer_p(list)
        && doubly_linked_list_structure_legal_p(*list)) {
        node = *list;
        do {
            next = node->next;
...
static inline bool
doubly_linked_list_contains_p_i(struct doubly_linked_list *list,
    struct doubly_linked_list *node)
{
    ...
    struct doubly_linked_list *node)
{
    if (!doubly_linked_list_structure_legal_p(list)) {
        return false;
    } else if (!doubly_linked_list_structure_legal_p(node)) {
        return false;
    } else {
        return doubly_linked_list_contains_p_i(list, node);
    }
}
...
doubly_linked_list_remove(struct doubly_linked_list **list)
{
    if (complain_null_pointer_p(list)) {
        return PTR_INVALID;
    } else if (!doubly_linked_list_structure_legal_p(*list)) {
    } else {
    ...
    struct doubly_linked_list *n)
{
    ...
    // Fix-Me
    if (!doubly_linked_list_structure_legal_p(m)
        && !doubly_linked_list_structure_legal_p(n)) {
        return PTR_INVALID;
    } else if (!doubly_linked_list_structure_legal_p(m)) {
        return n;
    } else if (!doubly_linked_list_structure_legal_p(n)) {
        return m;
    } else if (m == n) {
